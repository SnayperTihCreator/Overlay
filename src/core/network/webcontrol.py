...
class ClientWebSockets:
    ...
    async def asend_message(self, message):
        async with websockets.connect(
                f"ws://{self.ip}:{self.port}", ping_interval=None,
                extensions=None, open_timeout=5, additional_headers={
                    "User-Agent": "Python/3.11 websockets/15.0",
                    "Origin": "http://127.0.0.1"
                }) as client:
            await client.send(message)
            try:
                return await asyncio.wait_for(client.recv(), timeout=10.0)
            except asyncio.TimeoutError:
                return "[red]Ошибка: Сервер принял команду, но не ответил вовремя.[/red]"
